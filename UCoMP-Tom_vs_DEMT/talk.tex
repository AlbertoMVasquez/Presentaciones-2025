...
\noindent\hrulefill
\saltito
\centerline{\footnotesize Summary of the techniques:}
\saltito
...
\noindent\hrulefill
\saltito
\centerline{\footnotesize Results shown in these slides:}
\saltito
...
\begin{center}
\includegraphics[width=0.45\textwidth,clip=]{fig/comparison_ucomp_vs_aia_Sept2022_global_ratio_range1_090-1_200_Rsun.eps}
\includegraphics[width=0.45\textwidth,clip=]{fig/comparison_kcor_vs_ucomp_Sept2022_global_ratio_range1_090-1_200_Rsun.eps}
\includegraphics[width=0.45\textwidth,clip=]{fig/Average_Radial_Profiles_ucomp_vs_aia_Sept2022_global.eps}
\includegraphics[width=0.45\textwidth,clip=]{fig/Average_Radial_Profiles_kcor_vs_ucomp_Sept2022_global.eps}
\end{center}
}
}
...
\salto
\item Emission lines to which UCoMP and AIA are produced by $\rm Fe$ ions. While the UCoMP-SRT $\Ne$ is independent of $\rm[Fe]$, AIA-DEMT $\Ne \propto 1/ \sqrt{\rm [Fe]}$. Comparison of their reconstructed $N_e$ can in principle provide constraints on the 3D distribution of $\rm[Fe]$, as well as the coronal filling factor affecting emission lines.
\salto
\item The larger difference of KCOR-SRT $\Ne$ compared to AIA-DEMT $\Ne$ (and therefore also compared to UCoMP-SRT $\Ne$) has been consistently found for other periods (\azul{Lloveras et al. 2019}). Discrepancy can be due to calibration issues, coronal $[\rm Fe]$, and/or coronal filling factor.
\salto
\item In these plots we simply carried out a voxel-by-voxel comparison within the whole reconstructed volume and not along specific field lines, which we can do next.
\end{itemize}
}
}
\end{document}
